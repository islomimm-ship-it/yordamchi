...
SETTINGS_MENU = ReplyKeyboardMarkup(
    [["⏰ Eslatma vaqtini o'zgartirish"], ["⬅️ Orqaga"]],
    resize_keyboard=True,
)
...
# --- Maqsadlar ---
def add_goal(chat_id, title, deadline):
    conn = get_conn()
    cur = conn.cursor()
    cur.execute(
...
def build_daily_message(chat_id: int) -> str:
    ...
    if debts:
        ...
        for d in debts:
            alert = " ⚠️ <b>BUGUN TO'LOV KUNI!</b>" if d["due_day"] and d["due_day"] == today_day_num else ""
            lines.append(
    ...
    if not any([pinned, ishlar, soglik, sport, tasks, debts]) and total_exp == 0:
...
async def credit_add_start(update, context):
    ...
    await update.message.reply_text("Kimga / kimdan kredit? Ismini yozing:", reply_markup=CANCEL_MENU)
    return CR_PERSON


async def credit_person(update, context):
    context.user_data["cr_person"] = update.message.text.strip()
    await update.message.reply_text("Summasini kiriting (masalan 500000):", reply_markup=CANCEL_MENU)
    return CR_AMOUNT
...
async def pay_credit_start(update, context):
    ...
    await update.message.reply_text("Qaysi kredit uchun to'lov kiritamiz?", reply_markup=InlineKeyboardMarkup(buttons))
    return PAY_SELECT
...
async def expense_add_start(update, context):
    ensure_user(update.effective_chat.id)
    await update.message.reply_text("Xarajat summasini kiriting (masalan 25000):", reply_markup=CANCEL_MENU)
    return EXP_AMOUNT
...
async def expense_amount(update, context):
    ...
    await update.message.reply_text("Qaysi toifaga tegishli?", reply_markup=InlineKeyboardMarkup(buttons))
    return EXP_NOTE
...
async def goal_add_start(update, context):
    ensure_user(update.effective_chat.id)
    await update.message.reply_text("Maqsad sarlavhasini yozing:", reply_markup=CANCEL_MENU)
    return GOAL_TITLE
...
async def finalize_goal(chat_id, context, deadline, edit_query=None, message=None):
    ...
    add_goal(chat_id, title, deadline)
...
async def show_goals_list(update, context):
    ...
    for g in goals:
        ...
        lines.append(f"• {g['title']} ({g['deadline']}) — {status}")
...
async def routine_title(update, context):
    ...
    await update.message.reply_text("Qachon / qanday tartibda? (masalan: Har kuni ertalab):", reply_markup=CANCEL_MENU)
...
def main():
    ...
    credit_conv = ConversationHandler(
        ...
        states={
            ...
            CR_AMOUNT: [MessageHandler(filters.TEXT & ~filters.COMMAND, credit_amount)],
            CR_DUE_DAY: [
        ...
        fallbacks=[MessageHandler(filters.Regex("^❌ Bekor qilish$"), cancel_conv)],
    )
    ...
    pay_conv = ConversationHandler(
        ...
        states={
            ...
            PAY_AMOUNT: [MessageHandler(filters.TEXT & ~filters.COMMAND, pay_credit_amount)],
        },
        fallbacks=[MessageHandler(filters.Regex("^❌ Bekor qilish$"), cancel_conv)],
    )
    ...
    routine_conv = ConversationHandler(
        ...
        states={
            ...
            ROUTINE_TITLE: [MessageHandler(filters.TEXT & ~filters.COMMAND, routine_title)],
            ROUTINE_SCHEDULE: [MessageHandler(filters.TEXT & ~filters.COMMAND, routine_schedule)],
        },
        fallbacks=[MessageHandler(filters.Regex("^❌ Bekor qilish$"), cancel_conv)],
    )
    ...
    for conv in (credit_conv, pay_conv, expense_conv, budget_conv, task_conv, goal_conv, pinned_conv, routine_conv, time_conv):
