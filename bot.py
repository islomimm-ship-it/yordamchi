...
async def income_add_start(update, context):
    ensure_user(update.effective_chat.id)
    await update.message.reply_text("Daromad summasini kiriting (masalan 3000000):", reply_markup=CANCEL_MENU)
    return INC_AMOUNT
...
def main():
    ...
    for conv in (credit_conv, pay_conv, expense_conv, budget_conv, income_conv, task_conv, goal_conv, pinned_conv, routine_conv, time_conv):
